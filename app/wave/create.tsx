...
export default function CreateWaveScreen() {
  ...
  const locationValid = isOnline
    ? meetingUrl.length > 0
    : locationName.length > 0 && latitude.length > 0 && longitude.length > 0;
    ...
    try {
      ...
      const location = isOnline
        ...
        : {
            ...
            latitude: parseFloat(latitude) || 35.68,
            longitude: parseFloat(longitude) || 139.76,
            is_online: false,
          };
      ...
      if (media && wave) {
        try {
          ...
          const { error: uploadErr } = await supabase.storage
            .from("clips").upload(fileName, blob, {
          ...
          if (!uploadErr) {
            ...
            await supabase.from("clips").insert({
              wave_id: wave.id, user_id: user.id, media_url: publicUrl,
              thumbnail_url: publicUrl, caption: clipCaption || "",
              duration_sec: media.duration ? Math.min(Math.round(media.duration / 1000), 15) : 0,
      ...
      showAlert("波を作成しました", "フィードとマップに反映されます");
      router.back();
    } catch (err: any) {
  ...
  return (
    <ScrollView style={s.container} contentContainerStyle={s.content} keyboardShouldPersistTaps="handled">
      ...
      {isOnline ? (
        ...
        <>
          <Text style={s.label}>場所の名前（必須）</Text>
          <TextInput style={s.input} value={locationName} onChangeText={setLocationName} placeholder="例: 砧公園 東側広場" placeholderTextColor={Colors.textLight} />

          <Text style={s.label}>住所</Text>
          <TextInput style={s.input} value={address} onChangeText={setAddress} placeholder="例: 東京都世田谷区砧公園1-1" placeholderTextColor={Colors.textLight} />

          <View style={s.row}>
            <View style={s.half}>
              <Text style={s.label}>緯度（必須）</Text>
              <TextInput style={s.input} value={latitude} onChangeText={setLatitude} placeholder="35.6324" placeholderTextColor={Colors.textLight} keyboardType="decimal-pad" />
            </View>
            <View style={s.half}>
              <Text style={s.label}>経度（必須）</Text>
              <TextInput style={s.input} value={longitude} onChangeText={setLongitude} placeholder="139.6142" placeholderTextColor={Colors.textLight} keyboardType="decimal-pad" />
            </View>
          </View>
          <Text style={s.hint}>Google Mapsで場所を長押しすると座標をコピーできます</Text>
        </>
      )}
